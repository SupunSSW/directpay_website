...
use App\Mail\Backend\SendUserCredentials;
...
class UserRepository extends BaseRepository
{
    ...
    {
        return $this->model->getActionButtonsAttribute();
    }
    ...
    public function create(array $data) : User
    {
        return DB::transaction(function () use ($data) {
            ...
            $fName1st = substr($data['first_name'], 0, 2);
            $lName2nd = substr($data['last_name'], 0, 2);
            $rand = rand(1111,9999);
...
//            $userObj->url = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
//
//            //sendEmail($data['email'], $password);
//            Mail::to($data['email'])->send(new SendUserCredentials($userObj));

            $user = parent::create([
                'first_name' => $data['first_name'],
                'last_name' => $data['last_name'],
                'email' => $data['email'],
                'agent_hub' => $data['agent_hub'] == "0" ? null : $data['agent_hub'],
                'password' => $password,
                'active' => isset($data['active']) && $data['active'] == '1' ? 1 : 0,
                'confirmation_code' => md5(uniqid(mt_rand(), true)),
                'confirmed' => isset($data['confirmed']) && $data['confirmed'] == '1' ? 1 : 0,
